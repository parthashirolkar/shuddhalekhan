import './AgentResponsePopup.css';

interface AgentResponsePopupProps {
  message: string;
}

export function AgentResponsePopup({ message }: AgentResponsePopupProps) {
  return (
    <div className="agent-response-overlay">
      <div className="agent-response-container">
        <div className="agent-response-icon">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3Z"/>
            <path d="M19 10v2a7 7 0 0 1-14 0v-2"/>
            <line x1="12" y1="19" x2="12" y2="23"/>
            <line x1="8" y1="23" x2="16" y2="23"/>
          </svg>
        </div>
        <div className="agent-response-content">
          <span className="agent-label">Jarvis</span>
          <p className="agent-message">{message}</p>
        </div>
      </div>
    </div>
  );
}
