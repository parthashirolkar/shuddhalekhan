...
export class AudioRecorder {
	...
	async initialize(deviceId?: string): Promise<AudioRecorderResult> {
		...
		if (deviceId) {
			try {
				...
				logger.warning(
					`Failed to initialize with device '${deviceId}': ${error instanceof Error ? error.message : String(error)}`,
				);
		...
		try {
			...
			logger.error(
				`Failed to initialize with default device: ${error instanceof Error ? error.message : String(error)}`,
			);
			throw error;
		}
	}
			...
			(data: Float32Array) => {
				...
				if (this.isRecording) {
					if (!this.firstCallbackTime) {
						...
						const callbackDelay =
							this.firstCallbackTime - (this.streamStartTime ?? 0);
						logger.info(
							`[PERF] First audio callback fired after ${callbackDelay.toFixed(0)}ms`,
						);
					}
	...
	private downmixToMono(stereoData: Float32Array) {
		const monoData = Float32Array.from(
			{ length: stereoData.length / 2 },
			(_, i) => {
