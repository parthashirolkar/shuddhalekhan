...
type RecordingState = "idle" | "recording" | "transcribing" | "agent_mode";
	...
	async initialize(): Promise<void> {
		const menuItems: MenuItemClickable[] = [
			...
			{ title: "-", tooltip: "", checked: false, enabled: false },
			{
				...
				tooltip: "Toggle AI agent mode for voice commands",
				checked: false,
				enabled: true,
				click: () => this.handleToggleAgentMode(),
			},
			{ title: "-", tooltip: "", checked: false, enabled: false },
			{
	...
	private async handleChooseDevice(): Promise<void> {
		try {
			...
			const deviceItems = devices
				.map(
					(d) =>
						`        $listBox.Items.Add('${d.name.replace(/'/g, "''")}${
							d.isDefault ? " (Default)" : ""
						}') | Out-Null`,
				)
				.join("\n");
			...
			const selectIndex =
				currentDeviceId &&
				devices.findIndex((d) => d.id === currentDeviceId) >= 0
...
`;
			...
			try {
				...
				const selectedIndex = Number.parseInt(stdout.trim(), 10);
	...
	private async handleAbout(): Promise<void> {
		const hotkeysText = this.config.agent?.enabled
			? "Standard: Ctrl+Win\nAgent: Ctrl+Alt+Win"
			: "Ctrl+Win";
	...
	private async handleToggleAgentMode(): Promise<void> {
		const currentState = this.config.agent?.enabled ?? false;
		const newState = !currentState;

		this.config.agent = this.config.agent || {};
		this.config.agent.enabled = newState;
		this.saveConfig(this.config);

		await this.showNotification(
			"Agent Mode",
			newState
				? "Agent mode enabled. Press Ctrl+Alt+Win to use voice commands."
				: "Agent mode disabled.",
		);

		logger.info(`Agent mode ${newState ? "enabled" : "disabled"}`);
		this.updateAgentModeMenuItem();
	}

	private updateAgentModeMenuItem(): void {
		if (!this.systray) {
			return;
		}

		const enabled = this.config.agent?.enabled ?? false;
		const menuItem = {
			title: `Agent Mode: ${enabled ? "Enabled" : "Disabled"}`,
			tooltip: "Toggle AI agent mode for voice commands",
			checked: false,
			enabled: true,
		};
